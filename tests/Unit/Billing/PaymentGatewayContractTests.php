...
trait PaymentGatewayContractTests
{
    ...
    function charges_with_valid_payment_token_are_successful()
    {
        ...
        $newCharges = $paymentGateway->newChargesDuring(function($paymentGateway)  {
            $paymentGateway->charge(3000, $paymentGateway->getValidTestToken(), 'test_account_00');
        });
    ...
    function can_get_details_about_successful_charge()
    {
        ...
        $charge = $paymentGateway->charge(3000, $paymentGateway->getValidTestToken($paymentGateway::TEST_CARD_NUMBER), 'test_account_001');

        $this->assertEquals(substr($paymentGateway::TEST_CARD_NUMBER, -4), $charge->cardLastFour());
        $this->assertEquals(3000, $charge->amount());
        $this->assertEquals('test_account_001', $charge->destination());
    }
    ...
    function charges_with_invalid_payment_token_fails()
    {
        ...
        $newCharges = $paymentGateway->newChargesDuring(function($paymentGateway) {
            try {
                $paymentGateway->charge(2500, 'invalid-token', 'test_account_00');
    ...
    function can_fetch_charges_created_during_callback()
    {
        ...
        $paymentGateway->charge(2000, $paymentGateway->getValidTestToken(), 'test_account_00');

        $newCharges = $paymentGateway->newChargesDuring(function($paymentGateway)  {
            $paymentGateway->charge(3000, $paymentGateway->getValidTestToken(), 'test_account_00');
            $paymentGateway->charge(4000, $paymentGateway->getValidTestToken(), 'test_account_00');
            $paymentGateway->charge(5000, $paymentGateway->getValidTestToken(), 'test_account_00');
        });
